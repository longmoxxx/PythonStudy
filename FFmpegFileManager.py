...
def split_movie(input_video_path,output_video_path):
    ...
    #以视频名称创建目录
    output_video_path = output_video_path+file_name
    ...
    #play_list文件输出路径
    play_list_out = output_video_path+'\\'+file_name+'_playlist.m3u8'
    # 编码格式
    code_type=' -b:v 4M -minrate 4M -maxrate 4M -bufsize 4M ' \
              '-codec:v libx264 -codec:a mp3 -map 0 -f ssegment -segment_format mpegts -segment_list '+play_list_out+' -segment_time 10 '
    #执行命令
    cmd=ffmpeg_path+input_video_path+code_type+output_video_path+'\\'+file_name+'_%03d.ts'
    print(cmd)
    subprocess.call(cmd)
